...
if __name__=='__main__':
    # load docs 
    data_name='game'
    df = pd.read_csv(f'data/{data_name}/{data_name}_train.csv')
    df = df.dropna()
    ...
    kw_df.to_csv(f'data/{data_name}/keybert_train_item_keywords.csv')
    ...
    keywords = keybert_extractor.get_keywords(duplicate_limit=0.1, num_keywords=500)
    kw_df = pd.DataFrame({'keyword':list(keywords)})
    kw_df.to_csv(f'data/{data_name}/keybert_train_user_keywords.csv')